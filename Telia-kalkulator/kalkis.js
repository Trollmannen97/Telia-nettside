...
function calculatePrice() {
  ...
  function calculateTotalPrice() {
    ...
    familyPlans.forEach(function (plan) {
      ...
      if (planText.includes("10GB")) {
        ...
        planPrice -= 30; // Trekk fra familierabatten
        console.log(
          "Telia 10 GB etter rabatt (30 kr trukket fra): ",
          planPrice
        );
      }
  ...
  // Funksjon for å vise/skjule familiefelt basert på kundetype.bæsk
  function toggleCustomerType() {
  ...
  function calculatePrice() {
    ...
    if (isFamily) {
      ...
      for (var i = 0; i < familyPlans.length; i++) {
        ...
        // Legg til familiemedlemmets plan i resultatlisten
        detailedResult += `<p>Nummer ${i + 2}: ${
          familyPlans[i].options[familyPlans[i].selectedIndex].text.split(
            "-"
          )[0]
        } - ${discountedFamilyPrice.toFixed(2)} kr</p>`;
      }
